module module_types_cuda
  ...
  attributes(device) subroutine update(s2,s0,tend,dt)
    ...
    ! NOTA MHPC: Mancano indici i, k, ll qui, assumo siano calcolati o passati nel contesto reale
    ! Se questo è un device routine chiamato da un kernel, ok.
  ...
  attributes(device) subroutine xtend(tendency,flux,ref,atmostat,dx,dt)
    ...
    ! MHPC FIX: Chiamata statica diretta invece di atmostat%exchange_halo_x()
  ...
  attributes(device) subroutine ztend(tendency,flux,ref,atmostat,dz,dt)
    ...
    ! MHPC FIX: Chiamata statica diretta
    ! NOTA: exchange_halo_z contiene MPI, fallirà su device, vedi soluzione ottimale.
    call exchange_halo_z(atmostat, ref)
  ...
  attributes(device) subroutine exchange_halo_x(s)
    ...
    attributes(device) subroutine exchange_halo_z(s,ref)
        ...
        ! ATTENZIONE: MPI CALLS ON DEVICE ARE INVALID IN STANDARD CUDA FORTRAN
        ! Questo codice compila solo perché il compilatore potrebbe non validare le interfacce MPI all'interno
        ! di attributi device finché non linka, ma crasherà a runtime.
...
! ... (resto delle subroutine new/del invariate)
