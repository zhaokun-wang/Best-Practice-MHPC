module module_physics_cuda
  ...
  attribute(global) subroutine init(etime,output_counter,dt)
    implicit none
    ...
    grid_dims = dim3(nx/16 + 1, nz_loc/8 + 1, NVARS)
    block_dims = dim3(16, 8, 1)

    i = (blockIdx%x - 1) * blockDim%x + threadIdx%x - hs
    k = (blockIdx%y - 1) * blockDim%y + threadIdx%y - hs
    ll = (blockIdx%z - 1) * blockDim%z + threadIdx%z - hs
    ...
    call tend%new_tendency<<<grid_dims, block_dims>>>( )
    call ref%new_ref<<<grid_dims, block_dims>>>( )
